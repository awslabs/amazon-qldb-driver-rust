...
pub trait TransactionRetryPolicy {
    fn on_err(&self, error: &CommunicationError, attempt_number: u32) -> RetryInstructions;
}
...
impl TransactionRetryPolicy for NeverRetryPolicy {
    fn on_err(&self, _error: &CommunicationError, _attempt_number: u32) -> RetryInstructions {
...
impl TransactionRetryPolicy for ExponentialBackoffJitterTransactionRetryPolicy {
    fn on_err(&self, error: &CommunicationError, attempt_number: u32) -> RetryInstructions {
        let should_retry = match error {
            CommunicationError::Transport(transport) => match transport {
            ...
            CommunicationError::UnexpectedResponse { .. } => true,
            CommunicationError::MalformedResponse { .. } => true,
        };
